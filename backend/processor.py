...
def split_name_first_last(fullname: str) -> tuple:
    """Wrapper para separar primeiro nome e sobrenome usando a heurística
    de `separar_nome_sobrenome`, aplicando sanitização e limites de tamanho.
    """
    if not fullname:
        return "", ""

    # Usar heurística rica para separar nome/sobrenome
    first, last = separar_nome_sobrenome(fullname)
    ...
    try:
        ...
        for col in out_df.columns:
            if out_df[col].dtype == object:
                # garantir string e aplicar upper; dígitos permanecem inalterados
                out_df[col] = out_df[col].fillna("").astype(str).str.upper()
