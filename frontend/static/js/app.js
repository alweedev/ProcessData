document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderPreviewStatsHtml(stats){
    ...
    for(const [k,v] of Object.entries(stats)){
      ...
      const label = key.includes('cpf') ? 'CPF' : key.includes('exact') ? 'Exato' : key.includes('token') ? 'Token' : key.includes('fuzzy') ? 'Fuzzy' : k;
      // Valores primitivos são apresentados como badges
      if(v === null || typeof v === 'number' || typeof v === 'string' || typeof v === 'boolean'){
        parts.push(`<span class="badge bg-info text-dark me-1 preview-badge">${label}: ${v}</span>`);
      } else {
        // Para estruturas complexas (arrays/objetos) exibir um resumo compacto
        try{
          if (Array.isArray(v)) {
            parts.push(`<div class="mt-2"><strong>${label}:</strong> <span class="small text-muted">array[${v.length}]</span></div>`);
          } else if (typeof v === 'object') {
            const keys = Object.keys(v || {}).slice(0,5).join(', ');
            parts.push(`<div class="mt-2"><strong>${label}:</strong> <span class="small text-muted">object{${keys}${Object.keys(v||{}).length>5?', ...':''}}</span></div>`);
          } else {
            parts.push(`<div class="mt-2"><strong>${label}:</strong> ${String(v)}</div>`);
          }
        }catch(e){
          parts.push(`<div class="mt-2"><strong>${label}:</strong> ${String(v)}</div>`);
        }
    ...
    return `<div class="mt-3"><h6>Estatísticas</h6><div>${parts.join('')}</div></div>`;
  }
  ...
  function renderInactiveMatchesHtml(inactiveObj){
    ...
    parts.push(`<div class="inativos-controls"><button type="button" class="btn btn-sm btn-outline-primary" id="inativos_expand_all">Expandir todos</button><button type="button" class="btn btn-sm btn-outline-secondary d-none" id="inativos_collapse_all">Colapsar todos</button><div class="ms-auto small text-muted">Clique em cada grupo para ver a amostra</div></div>`);
    parts.push('<div class="accordion inativos-accordion" id="inativosAccordion">');
    let idx = 0;
    for(const [group, arr] of Object.entries(inactiveObj)){
      if(!Array.isArray(arr) || arr.length === 0) { idx++; continue; }
      const sample = arr.slice(0, 10);
      // descobrir colunas relevantes (priorizar CPF, NomeCompleto, Status, Email, Departamento, NomeEmpresa)
      const preferred = ['CPF', 'CPFdigits', 'NomeCompleto', 'Nome Normalizado', 'Status', 'Email', 'Departamento', 'NomeEmpresa', 'UserId'];
      const cols = [];
      ...
      preferred.forEach(k => { if(keySet.has(k)) { cols.push(k); keySet.delete(k); } });
      // append remaining keys (limit to avoid huge tables)
      Array.from(keySet).slice(0, 8).forEach(k => cols.push(k));
      ...
      html += '<table class="table table-sm table-bordered"><thead class="table-light"><tr>' + cols.map(c => `<th>${c}</th>`).join('') + '</tr></thead><tbody>';
      for(const row of sample){
        html += '<tr>' + cols.map(c => `<td>${(row && row[c]) ? String(row[c]) : ''}</td>`).join('') + '</tr>';
      }
      html += '</tbody></table>';
      if(arr.length > sample.length) html += `<div class="small text-muted">Mostrando ${sample.length} de ${arr.length} itens. Exporte para análise completa.</div>`;
      html += '</div>'; // wrapper
  ...
  async function postFiles(url, filesInput, extra = {}, single = false) {
    ...
    if (single) {
      ...
      if (!filesInput || !filesInput.base || !filesInput.base.files || !filesInput.base.files[0]) {
      ...
      if (filesInput.lista && filesInput.lista.files && filesInput.lista.files[0]) {
  ...
  const fullUrl = (window.API_BASE && url.startsWith("/")) ? window.API_BASE + url : url;
        ...
        if (blob) {
          ...
          reader.onload = () => {
            ...
            try {
              ...
              const details = obj.errors ? ` | details: ${JSON.stringify(obj.errors)}` : "";
              reject(new Error(`Server ${xhr.status}: ${msg}${details}`));
            } catch (e) {
              // não JSON
              reject(new Error(`Server ${xhr.status}: ${text || xhr.statusText}`));
            }
          };
          reader.onerror = () => reject(new Error(`Server ${xhr.status}: ${xhr.statusText}`));
  ...
  async function postFormDataJson(url, formData) {
    ...
    try {
      ...
      // se não for JSON
      if (!resp.ok) throw new Error(`Server ${resp.status}: ${text || resp.statusText}`);
      return {};
    }
  }
  /**
  ...
  function showToast(message, type = "success") {
    ...
    const bg = type === 'success' ? 'success' : (type === 'info' ? 'info' : 'danger');
  ...
  if (apiStatusBtn) {
    ...
    apiStatusIcon?.classList.add("spin-rotating");
      try {
        const healthUrl = (window.API_BASE) ? window.API_BASE + "/" : "http://127.0.0.1:5000/";
  ...
  function validateAndShowFiles(filesInput) {
    ...
    try {
      ...
      if (display) display.textContent = Array.from(filesInput.files).map(f=>f.name).join(', ');
    } catch(e){/* no-op */}
  }
  ...
  if (inativacaoBaseUploadArea && inativacaoBase) {
    ...
    inativacaoBaseUploadArea.addEventListener('keydown', (e)=>{
      if(e.key === 'Enter' || e.key === ' '){ e.preventDefault(); inativacaoBase.click(); }
    });
  ...
  if (inativacaoListaUploadArea && inativacaoLista) {
    ...
    inativacaoListaUploadArea.addEventListener('keydown', (e)=>{
      if(e.key === 'Enter' || e.key === ' '){ e.preventDefault(); inativacaoLista.click(); }
    });
  ...
  const fdPreview = new FormData();
        ...
        if (extraData.lista_text) fdPreview.append("lista_text", extraData.lista_text);
  ...
  if (useFuzzyCheckbox) fdPreview.append('use_fuzzy', useFuzzyCheckbox.checked ? 'true' : 'false');
  if (fuzzyCutoffInput) fdPreview.append('fuzzy_cutoff', fuzzyCutoffInput.value || '0.90');
        let preview;
        try {
          preview = await postFormDataJson("/api/preview_inativacao", fdPreview);
        } catch (errPreview) {
          throw new Error(errPreview.message || "Erro no preview");
        }
        try { console.debug('preview payload', preview); } catch(e) {}
        ...
        if((!sample || sample.length === 0) && preview.stats && preview.stats.inactive_matches){
          try{
            const inactiveObj = preview.stats.inactive_matches || {};
            const all = [];
            for(const [type, arr] of Object.entries(inactiveObj)){
              if(Array.isArray(arr)){
                for(const it of arr){ all.push(Object.assign({match_type: type}, it)); }
              }
            }
            if(all.length){
              count = all.length;
            ...
            console.debug('fallback build sample from inactive_matches failed', e);
          }
        }

        // Preferir stats.total_matches quando presente (fonte de verdade do backend)
        const displayCount = (preview.stats && typeof preview.stats.total_matches === 'number') ? preview.stats.total_matches : count;
        ...
        if (sample && sample.length) {
          ...
          const rowsHtml = sample.slice(0,10).map(row => {
        ...
        if (Array.isArray(preview.records) && preview.records.length) {
          ...
          const preferred = ['Login','UserId','NomeCompleto','Email','Departamento','NomeEmpresa','CodigoCCustoEmpresa','Solicitante'];
          const cols = Array.isArray(preview.columns) ? preview.columns.slice() : Object.keys(preview.records[0] || {});
          const colsToShow = [];
          preferred.forEach(c => { if (cols.includes(c)) { colsToShow.push(c); cols.splice(cols.indexOf(c),1); } });
          // completar com primeiras colunas restantes (até 8 totais)
          while (colsToShow.length < 8 && cols.length) colsToShow.push(cols.shift());
          const initialRows = preview.records.slice(0, 200);
          const headerHtml = colsToShow.map(c => `<th>${c}</th>`).join('');
          const bodyHtml = initialRows.map(row => `<tr>${colsToShow.map(c => `<td>${row[c] ?? ''}</td>`).join('')}</tr>`).join('');
          infoHtml += `
            <div class="mt-3">
  ...
  // attach stats breakdown (if returned by backend)
        if(preview.stats){
          ...
          if(preview.stats.inactive_matches){
            ...
            try{
              const counts = Object.fromEntries(Object.entries(inact).map(([k,v])=>[k, Array.isArray(v)?v.length:0]));
              const summary = `<div class="alert alert-warning"><strong>Atenção:</strong> Foram encontradas correspondências que já estão <strong>INATIVAS</strong> na base.<div class="mt-2 small">Resumo: ${Object.entries(counts).map(([k,c])=>`${k}: ${c}`).join(' | ')}</div></div>`;
              infoHtml = summary + infoHtml;
            }catch(e){
              infoHtml = `<div class="alert alert-warning"><strong>Atenção:</strong> Foram encontradas correspondências já INATIVAS.</div>` + infoHtml;
            }
            // agora adicionar tabelas legíveis para cada grupo
              infoHtml += renderInactiveMatchesHtml(preview.stats.inactive_matches);
          }
          ...
          function objectsToCsv(objs){
            ...
            const lines = objs.map(o => keys.map(k => escape(o[k] ?? '')).join(','));
              ...
              try {
                ...
                if(btn){
                  btn.addEventListener('click', ()=>{
                    try{
                      const inactive = preview.stats && preview.stats.inactive_matches ? preview.stats.inactive_matches : {};
                      ...
                      if(!all.length){ showToast('Não há correspondências inativas para exportar.', 'info'); return; }
                    ...
                    }catch(e){ showToast('Falha ao gerar CSV: '+(e.message||e), 'danger'); }
                  });
                }
                ...
                try {
                  ...
                  const exportPreviewBtn = popup.querySelector('#export_preview_csv');
                  if (tbody && Array.isArray(preview.records) && preview.records.length) {
                    ...
                    const render = (rows) => {
                      ...
                      tbody.innerHTML = limited.map(row => `<tr>${shownCols.map(c => `<td>${row[c] ?? ''}</td>`).join('')}</tr>`).join('');
                    };
                    if (searchEl) {
                      searchEl.addEventListener('input', (e) => {
                        const term = (e.target.value||'').toLowerCase();
                        if (!term) { render(rowsAll); return; }
                        const filtered = rowsAll.filter(r => shownCols.some(c => String(r[c] ?? '').toLowerCase().includes(term)));
                        render(filtered);
                      });
                    }
                    render(rowsAll);
                ...
                } catch(e){ console.debug('preview table wiring error', e); }
              }catch(e){ console.debug('didOpen export control error', e); }
            }
          });
        ...
        if (useFuzzyCheckbox) extraData.use_fuzzy = useFuzzyCheckbox.checked ? 'true' : 'false';
        if (fuzzyCutoffInput) extraData.fuzzy_cutoff = fuzzyCutoffInput.value || '0.90';
  ...
  try {
    ...
    if (useFuzzy && fuzzyCutoff) {
      ...
      useFuzzy.addEventListener('change', ()=>{
        localStorage.setItem('fuzzy_prefs', JSON.stringify({use_fuzzy: useFuzzy.checked, fuzzy_cutoff: fuzzyCutoff.value}));
      });
  ...
  try{
    ...
    const storedPrefs = JSON.parse(localStorage.getItem('cadastro_prefs')||'{}');
    if(loginChoiceEl && storedPrefs.login_choice) loginChoiceEl.value = storedPrefs.login_choice;
    if(fluxoEl && storedPrefs.fluxo) fluxoEl.value = storedPrefs.fluxo;
    if(loginChoiceEl) loginChoiceEl.addEventListener('change', ()=>{
      const prefs = JSON.parse(localStorage.getItem('cadastro_prefs')||'{}');
      prefs.login_choice = loginChoiceEl.value; localStorage.setItem('cadastro_prefs', JSON.stringify(prefs));
    });
    if(fluxoEl) fluxoEl.addEventListener('change', ()=>{
      const prefs = JSON.parse(localStorage.getItem('cadastro_prefs')||'{}');
      prefs.fluxo = fluxoEl.value; localStorage.setItem('cadastro_prefs', JSON.stringify(prefs));
    });
  }catch(e){/* ignore */}
