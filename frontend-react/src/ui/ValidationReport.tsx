...
export function ValidationReport({ report, loading, error }: ValidationReportProps) {
  if (loading) {
    return (
      <div className="rounded-lg border border-border bg-surface-2 px-4 py-6 text-center text-sm text-text-muted">
        Validando planilha…
      </div>
    );
  }
  if (error) {
    return <div className="rounded-lg border border-border bg-surface-2 px-4 py-4 text-sm text-text-muted">{error}</div>;
  }
  ...
  return (
    <div className="space-y-3 rounded-lg border border-border bg-surface-2 p-4">
      ...
      {report.general_errors && (
        <div className="rounded-md bg-danger-soft px-3 py-2 text-sm text-danger">{report.general_errors}</div>
      )}
      ...
      {blanks.length > 0 && (
        <div className="text-sm text-text">
          ...
          <div className="flex flex-wrap gap-1.5">
            {blanks.map(([col, n]) => (
              <span key={col} className="rounded-full bg-warning-soft px-2 py-0.5 text-xs text-warning">
                {col}: {n}
              </span>
            ))}
          </div>
      ...
      {lineErrors.length > 0 && (
        <div className="text-sm text-text">
          ...
          <ul className="max-h-48 space-y-1 overflow-y-auto rounded-md bg-surface px-3 py-2 text-xs text-text-muted">
